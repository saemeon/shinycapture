...
ui <- fluidPage(
  ...
  p("Click to capture at 1200x800px regardless of browser size."),
  ...
  p("Same chart, decorations removed in the preprocess stage."),
  actionButton("cap_stripped", "Capture (stripped)"),
  uiOutput("preview_stripped"),
  hr(),

  h4("3. Table — capture via html2canvas"),
  p("Uses capture_element() with html2canvas strategy."),
  DTOutput("demo_table"),
  br(),
  ...
  p("Captures the chart and offers a download button."),
  ...
  selectInput("format_select", "Format:",
              c("png", "jpeg", "webp", "svg"), selected = "png"),
...
server <- function(input, output, session) {
  ...
  # --- 1. Capture at exact resolution ---
  observeEvent(input$cap_plot, {
    capture_plotly("scatter_plot",
      strategy = plotly_strategy(width = 1200, height = 800)
    )
  })

  observeEvent(input[[".shinycapture.scatter_plot"]], {
    # base64 data-URI — use directly as img src
    output$preview_plot <- renderUI(tagList(
      p(strong("Preview (1200x800):")),
      tags$img(src = input[[".shinycapture.scatter_plot"]],
  ...
  observeEvent(input[[".shinycapture.stripped"]], {
    output$preview_stripped <- renderUI(tagList(
      p(strong("Preview (title + legend stripped):")),
      tags$img(src = input[[".shinycapture.stripped"]],
               style = "max-width:600px; border:1px solid #ccc;")
    ))
  })
  ...
  observeEvent(input[[".shinycapture.for_download"]], {
    # Decode base64 to raw bytes for download
    captured_bytes(base64_decode(input[[".shinycapture.for_download"]]))
    output$download_ui <- renderUI(downloadButton("download_btn", "Download PNG"))
  })
  ...
  observeEvent(input$cap_format, {
    capture_plotly("scatter_plot",
      strategy = plotly_strategy(width = 1200, height = 800,
                                 format = input$format_select),
      input_id = ".shinycapture.format_demo"
    )
  })

  observeEvent(input[[".shinycapture.format_demo"]], {
    output$preview_format <- renderUI(tagList(
      p(strong(paste0("Captured as ", toupper(input$format_select), ":"))),
      tags$img(src = input[[".shinycapture.format_demo"]],
               style = "max-width:600px; border:1px solid #ccc;")
    ))
  })
}
